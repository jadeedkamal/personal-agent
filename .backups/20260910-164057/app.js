...
const musicUrlForm = $('#music-url-form');
const musicUrlInput = $('#music-url-input');
const musicDownloadBtn = $('#music-download-btn');
const musicStatus = $('#music-status');
const musicPlayerBar = $('#music-player-bar');
const musicPlayerTitle = $('#music-player-title');
const musicAudio = $('#music-audio');

function musicTrackTitle(name) {
  return name.replace(/\.mp3$/i, '');
}

async function loadMusicList() {
  musicList.innerHTML = '<div class="settings-loading">Loading…</div>';
  try {
    const r = await fetch('/api/music');
    if (!r.ok) throw new Error('request failed');
    const tracks = await r.json();
    if (!tracks.length) {
      musicList.innerHTML = '<div class="files-empty">No tracks yet — paste a link above.</div>';
      return;
    }
    musicList.innerHTML = '';
    for (const t of tracks) {
      ...
      row.className = 'files-row';
      row.innerHTML = `
        <span class="fname" title="${escapeHtml(t.name)}">${escapeHtml(musicTrackTitle(t.name))}</span>
        <span class="fmeta">${formatFileSize(t.size)}</span>
        <span class="factions">
          <button class="play-btn btn-reset" title="Play"><svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M7 5v14l11-7L7 5z" fill="currentColor"/></svg></button>
          <button class="dl-btn btn-reset" title="Download"><svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M12 4v11m0 0l-4-4m4 4l4-4M5 19h14" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
          <button class="del-btn btn-reset" title="Delete"><svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M6 6l12 12M18 6L6 18" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg></button>
        </span>`;
      row.querySelector('.play-btn').addEventListener('click', () => playTrack(t.name));
      row.querySelector('.dl-btn').addEventListener('click', () => {
        window.open(`/api/music/stream/${encodeURIComponent(t.name)}`, '_blank');
      });
      row.querySelector('.del-btn').addEventListener('click', async () => {
        if (!confirm(`Delete "${musicTrackTitle(t.name)}"?`)) return;
        await fetch(`/api/music/${encodeURIComponent(t.name)}`, { method: 'DELETE' });
        if (musicAudio.dataset.name === t.name) {
          musicAudio.pause();
    ...
    musicList.innerHTML = '<div class="settings-loading">Failed to load tracks</div>';
  }
}

function playTrack(name) {
  musicAudio.dataset.name = name;
  musicAudio.src = `/api/music/stream/${encodeURIComponent(name)}`;
  musicPlayerTitle.textContent = musicTrackTitle(name);
...
function setMusicStatus(text, isError) {
  ...
  musicStatus.classList.toggle('music-status-error', !!isError);
}

function openMusic() { musicModal.hidden = false; setMusicStatus(''); loadMusicList(); }
function closeMusic() { musicModal.hidden = true; }
...
document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && !musicModal.hidden) closeMusic(); });

musicUrlForm.addEventListener('submit', async (e) => {
  e.preventDefault();
  const url = musicUrlInput.value.trim();
  if (!url) return;
  musicDownloadBtn.disabled = true;
  setMusicStatus('Downloading…');
  try {
    const r = await fetch('/api/music/download', {
      method: 'POST',
    ...
    setMusicStatus(err.message || 'Download failed', true);
  } finally {
