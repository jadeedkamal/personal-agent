...
// ---------- MP3 player (download audio from a video link via yt-dlp) ----------

const MUSIC_DIR = path.join(os.homedir(), 'music');
fs.mkdirSync(MUSIC_DIR, { recursive: true });

const YTDLP_AUDIO_TIMEOUT_MS = 10 * 60 * 1000;
...
function runYtDlpAudio(url) {
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      '-x', '--audio-format', 'mp3',
      '-f', 'bestaudio/best',
      // Same bot-gate bypass as the video-downloader MCP tool: ride the desktop Chrome
      // profile's cookies and a JS runtime for YouTube's signature challenges.
    ...
    const timer = setTimeout(() => {
      child.kill('SIGKILL');
      reject(new Error(`Timed out after ${YTDLP_AUDIO_TIMEOUT_MS / 1000}s`));
    }, YTDLP_AUDIO_TIMEOUT_MS);
...
app.post('/api/music/download', requireAuth, async (req, res) => {
  ...
  try {
    const stdout = await runYtDlpAudio(parsed.toString());
    ...
    if (!resolved || !resolved.startsWith(MUSIC_DIR + path.sep) || !fs.existsSync(resolved)) {
      return res.status(500).json({ error: `Download finished but the output file could not be located (yt-dlp output: ${stdout.trim()})` });
    }
    const stat = fs.statSync(resolved);
    const name = path.basename(resolved);
    res.json({ name, size: stat.size, mtime: stat.mtimeMs });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

app.get('/api/music/stream/:name', requireAuth, (req, res) => {
  const name = path.basename(req.params.name);
...
app.delete('/api/music/:name', requireAuth, (req, res) => {
  const name = path.basename(req.params.name);
  const filePath = path.join(MUSIC_DIR, name);
  if (!fs.existsSync(filePath)) return res.status(404).json({ error: 'not found' });
  fs.unlinkSync(filePath);
...
// The music-player MCP tool calls this (localhost-only, same shared-secret gate as the routes
// above) so the agent can actually start playback in the user's browser instead of just telling
// them a file exists — it rides the same SSE channel as secure-input requests.
app.post('/internal/music/play', requireInternalSecret, (req, res) => {
  const name = path.basename(String((req.body || {}).name || ''));
  if (!fs.existsSync(path.join(MUSIC_DIR, name))) return res.status(404).json({ error: 'not found' });
  broadcastToAllGenerations('play_music', { name });
  res.json({ ok: true });
});
...
app.post('/api/chat/stream', requireAuth, (req, res) => {
  ...
  if (Array.isArray(attachments) && attachments.length) {
    // Absolute paths, not relative — Improve-mode conversations run with a different cwd
    // (the app's own source dir) than where uploads are stored, so a workspace-relative
    // path would silently fail to resolve there.
